...
module Test_Type_Graph = struct
  type node = ENode | SNode of string | INode of int
  ...
  let test_funcalls () =
    ...
    let gg =
      Graph.constrain gg fc
        (Graph.Call (Term fn.name, [ Term a.name; Term b.name ]))
    in
    Graph.showColor (5, 3, 2) gg;
  ...
  let test_funcalls_generic () =
    ...
    let gg =
      Graph.constrain gg fc
        (Graph.Call (Term fn.name, [ Type ("Ptr", []); Term c.name ]))
    in
    ...
    let gg =
      Graph.constrain gg ifblock
        (Graph.OneOf [ Graph.Term thenblock.name; Graph.Term elseblock.name ])
    in
    ...
    let gg =
      Graph.constrain gg elseblock
        (Graph.Call (Graph.Term fn.name, [ Graph.Term n.name ]))
    in
    let gg =
      Graph.constrain gg ifblock
        (Graph.OneOf [ Graph.Term thenblock.name; Graph.Term elseblock.name ])
    ...
    let gg =
      Graph.constrain gg fn
        (Graph.Type ("Func", [ Graph.Term n.name; Graph.Term ret.name ]))
    in
    Graph.showColor (5, 3, 2) gg;
    Graph.show @@ Graph.solve gg;
    ()
