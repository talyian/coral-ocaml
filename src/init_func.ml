...
let moduleExprAction = function
  | Module _ -> Noop
  | Func { name; ret_type = _rettype; _ } when name == ".init" -> IsMain
  | Func _ -> Noop
  | Multifunc _ -> Noop
  | TupleDef _ -> Noop
  | _ -> AddToMain
