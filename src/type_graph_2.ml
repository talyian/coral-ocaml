...
functor
  ...
  struct
    ...
    let rec cons_to_string = function
      ...
      | Call (a, p, _overload) ->
          cons_to_string a ^ "("
          ^ String.concat ", " (List.map cons_to_string p)
          ^ ")"
    ...
    let showColor rgb graph =
      let loop term cons =
        let rgb =
          if TermSet.mem term graph.active_terms then rgb else (2, 2, 2)
        in
        Printf.printf "%s :: %s\n"
    ...
    let rec findTerm graph name =
      ...
      | Some n -> Some n
      | None -> (
          match graph.parent with Some p -> findTerm p name | None -> None )

    let rec term_by_name graphs x =
      match graphs with
      | [] -> failwith ("term_by_name: " ^ x)
      | graph :: next -> (
          match findTerm graph x with Some x -> x | _ -> term_by_name next x )

    let getTermsByValue graph node =
      graph.terms
      |> StringMap.filter (fun _name { value = n; _ } ->
             Node.cmp n node = 0 && n <> Node.empty)
      |> StringMap.bindings

    let findTermByValue graph node =
      match
        graph.terms |> StringMap.bindings
        |> List.filter (function
             | _name, { value = n; _ }
               when Node.cmp n node = 0 && n <> Node.empty ->
                 true
             | _ -> false)
      with
      | [ (_, b) ] -> b
    ...
    let addTerm graph name node =
      let rec add_loop i =
        let newname =
          match i with 0 -> name | n -> name ^ "." ^ string_of_int n
        in
        match findTerm graph newname with
        | Some _ -> add_loop (i + 1)
        | None ->
            let term = { name = newname; value = node } in
            (term, { graph with terms = StringMap.add newname term graph.terms })
      in
      add_loop 0
    ...
    let rec simplify term =
      ...
      let uniq list =
        let remove x = function
          ...
          | [] -> []
        in
        match list with [] -> [] | x :: xs -> x :: remove x xs
      in
    ...
    let constrain graph term cons =
      if config.debug then
        Printf.printf "%s: %s :: %s\n"
          (Ansicolor.as_color (Bold GREEN) "Add ")
      ...
      | _ ->
          let update = function
            ...
            | Some x -> Some (simplify term @@ AllOf [ cons; x ])
          in
          {
            graph with
    ...
    let activate_replace_term term cons graph =
      ...
      let rec doReplace subject replacement =
        ...
        | OneOf p ->
            let c, p2 = recurseList p in
            (c, simplify subject @@ OneOf p2)
        | AllOf p ->
            let c, p2 = recurseList p in
            (c, simplify subject @@ AllOf p2)
      ...
      let fold_replace key target (count, actives, constraints) =
        ...
        let new_actives =
          match c with 0 -> actives | _ -> TermSet.add key actives
        in
        (count + c, new_actives, TermMap.add key new_cons constraints)
      in
      let cc, actives, constraints =
        TermMap.fold fold_replace graph.constraints
          (0, TermSet.empty, TermMap.empty)
      in
      ( cc,
        {
          graph with
          constraints;
          active_terms =
            TermSet.remove term @@ TermSet.union graph.active_terms actives;
        } )
    ...
    and nullify graph tt = function
      ...
      | Call (Type ("Func", _params), _args, _o) as call ->
          unify graph tt (Term tt.name) call
...
*)
    and unify graph tt cons1 cons2 =
      ...
      | (Term _ as cons1), Call (Type ("Func", params), args, _overload) ->
            ...
            | Free n ->
                let next =
                  ...
                  | None ->
                      ...
                      ( gg,
                        IntMap.add n new_term map,
                        Term new_term.name :: terms )
                  | Some t -> (gg, map, Term t.name :: terms)
                in
                next
            | Type (name, params) ->
                let gg, map, pterms =
                  List.fold_left instantiate_f (gg, map, []) params
                in
          ...
          let graph, _name, params =
            let g, _, t =
              instantiate_f (graph, IntMap.empty, []) (Type ("Func", params))
            in
            match t with
            | [ Type (n, p) ] -> (g, n, p)
            | _other -> failwith "mismatch in instantiate_f return"
          in
          ...
          if List.length arg1 <> List.length params then
            Fail
              (Printf.sprintf "type mismatch %s %s"
                 (cons_to_string (Type ("Func", params)))
                 (cons_to_string (Type ("Args", arg1))))
          else unify_zip graph tt arg1 params
      ...
      | (Type _ as y), Member (Type (typename, []), membername, index_cons) -> (
          ...
          | Some info, Some idx_ordinal_term ->
              let graph =
                ...
                | Term index_term ->
                    let indexterm =
                      Type ("Index", [ Term idx_ordinal_term.name ])
                    in
                    let graph =
                      constrain graph
                        (term_by_name [ graph ] index_term)
                        indexterm
                    in
                    {
                      graph with
                      active_terms =
                        TermSet.add idx_ordinal_term graph.active_terms;
                    }
                | _ -> failwith "unknown index holder"
              in
      ...
      | Term tname, Member (Type (typename, []), membername, index_cons) -> (
          ...
          | Some t, Some idx_ordinal_term ->
              ...
              let graph =
                {
                  graph with
                  active_terms = TermSet.add idx_ordinal_term graph.active_terms;
                }
              in
    ...
    let rec step graph term cons =
      ...
      let handle_unify_result = function
        ...
        | Success (graph, items) ->
            ...
            let graph =
              List.fold_left (fun gg (t, c) -> constrain gg t c) graph items
            in
            let newterms = List.map fst items |> TermSet.of_list in
            ( 1,
              {
                graph with
                active_terms = TermSet.union graph.active_terms newterms;
              } )
        | Defer _items ->
      ...
      | AllOf items -> (
          ...
          match
            List.find_opt
              (function Term _ -> true | Type _ -> true | _ -> false)
              items
          with
          | Some (Type (tt, tp) as cc) ->
              ...
              let unifold graph cons =
                ...
                | Fail _ -> graph
                | Success (gg, items) ->
                    List.fold_left (fun gg (t, c) -> constrain gg t c) gg items
                | Defer _items -> graph
              in
    ...
    let solve graph =
      ...
      let _i, g1 = solve_step 10 graph in
      finalize g1
  end
