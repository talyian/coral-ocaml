...
let parse name lexbuf =
  let m =
    try Grammar.main LexerInterface.token lexbuf
    with Error as exc ->
      ...
      raise exc
  in
  (match m with Module modinfo -> Ast.Module { modinfo with name } | x -> x)
  |> Multifunc.run |> Return_insert.run |> Init_func.run |> Name_resolver.run
  |> Type_resolver.run
