...
type compileContext =
  ...
  ; ns: Names.t
  ; ts: Coral_types.Resolver.resolver
  ; llvals: (Ast.node, Llvm.llvalue, Ast.Node.comparator_witness) Map.t }

type func_compile_ctx =
  {parent: compileContext; current_func: Llvm.llvalue; builder: Llvm.llbuilder; terminated: bool}
...
let codegen_at_any_level (data : compileContext) expr =
  ...
  | Ast.FloatLiteral f -> (data, Llvm.const_float_of_string (Llvm.double_type data.context) f)
...
let rec _codegen_func (fctx : func_compile_ctx) node =
  ...
  | Ast.Binop {callee; args} | Ast.Call {callee; args} -> (
    ...
    | Some (Ast.Builtin Builtins.LT, _) -> (
      ...
      | _ -> failwith "bad eq arguments" )
    | Some (Ast.Builtin Builtins.ADD_STR, _) -> (
      (* match List.fold_map ~init:fctx ~f:func_codegen args with
       * | fctx, [ lhs; rhs ] -> (fctx, Llvm.build_fadd lhs rhs "" fctx.builder)
       * | _ -> failwith "bad add arguments" ) *)
      match List.fold_map ~init:fctx ~f:func_codegen args with
      | fctx, [_lhs; rhs] -> (fctx, rhs)
      | _ -> failwith "bad add_str arguments" )
    | Some (Ast.Builtin Builtins.ADD_FLOAT, _) -> (
      match List.fold_map ~init:fctx ~f:func_codegen args with
      | fctx, [lhs; rhs] -> (fctx, Llvm.build_fadd lhs rhs "" fctx.builder)
      | _ -> failwith "bad add arguments" )
    | Some (Ast.Builtin Builtins.ADD_INT, _) -> (
      match List.fold_map ~init:fctx ~f:func_codegen args with
  ...
  | StringLiteral s -> (fctx, Llvm.build_global_stringptr s s fctx.builder)
  | FloatLiteral _ | IntLiteral _ -> (fctx, snd @@ codegen_at_any_level fctx.parent expr)
  | Ast.Let (_var, value) -> func_codegen fctx value
  | expr ->
...
let print_ir (ns : Names.t) (ts : Coral_types.Resolver.resolver) expr =
  let name, lines =
    match expr with
    | Ast.Module {name; lines}, _ -> (name, lines)
    | _ -> failwith "print_ir: expected module" in
  ...
  let data =
    {context; llmodule; main_builder; main_func= None; ns; ts; llvals= Map.empty (module Ast.Node)}
  in
  Stdio.printf "\n\n" ;
