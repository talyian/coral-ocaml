...
let rec constraint_to_type = function
  ...
  | Graph.Type (s, params) ->
      Ast.Parameterized (s, List.map constraint_to_type params)
  | c -> failwith ("unhandled type: " ^ Graph.cons_to_string c)

(* createGraph over a list of terms *)
let rec foldGraphr g terms =
  let folder (terms, g) node =
    let term, g = createGraph g node in
    (term :: terms, g)
  in
...
and createGraph g node =
  ...
  | Block list as block -> (
      ...
      | list ->
          ...
          let gg =
            Graph.constrain gg blockterm (Graph.Term (List.hd terms).name)
          in
          (blockterm, gg) )
  | Multifunc data as mfunc ->
      let rec get_terms gg data =
        ...
        let term, gg3 =
          ...
          | [ (_name, term) ] -> (term, gg2)
          | _ -> failwith ("multiple terms for " ^ data.name)
        in
        (term :: tail, gg3)
      in
      ...
      let graph =
        ...
        let options =
          List.map (fun (t : Graph.term) -> Graph.Term t.name) terms
        in
        let mfunc_type = Graph.OneOf options in
        Graph.constrain graph mfunc_term mfunc_type
      in
      (mfunc_term, graph)
  | Func fdata as func ->
      let terms, gg = foldGraphr g fdata.params in
      let terms =
        List.map (fun (x : Graph.term) -> Graph.Term x.name) @@ List.rev terms
      in
      ...
      let gg =
        ...
        | Ast.Type "" -> gg
        | _ -> Graph.constrain gg retTerm (type_to_constraint fdata.ret_type)
      in
      let gg =
        match fdata.body with
        | Empty -> gg
        | _ -> Graph.constrain gg retTerm (Graph.Term bodyTerm.name)
      in
      let gg =
        Graph.constrain gg fterm
          (Graph.Type ("Func", terms @ [ Graph.Term retTerm.name ]))
      in
      (fterm, gg)
  | Def info as def -> (
  ...
  | If (cond, ifbody, elsebody) as ifnode ->
      ...
      let gg =
        Graph.constrain gg ifterm
          (Graph.OneOf
             [ Graph.Term ifbodyterm.name; Graph.Term elsebodyterm.name ])
      in
      (ifterm, gg)
  | Return v as ret ->
  ...
  | Binop { name = op; args = [ lhs; rhs ]; _ } as binop ->
      ...
      let gg =
        ...
        Graph.constrain gg term
          (Graph.Call (callee, args, Graph.Term overload_idx.name))
      in
      (term, gg)
  | Var info -> (
  ...
  | Call { callee; args; _ } as call ->
      ...
      let gg =
        ...
        let c2 =
          List.map (fun (t : Graph.term) -> Graph.Term t.name) argterms
        in
        let overload_idx, gg =
          Graph.addTerm gg ("overload." ^ calleeterm.name) call
        in
        let inst_term, gg = Graph.addTerm gg ("inst." ^ calleeterm.name) call in
        let gg =
          Graph.constrain gg term
            (Graph.Call (c1, c2, Graph.Term overload_idx.name))
        in
        let inst_params = c2 @ [ Graph.Term term.name ] in
        let gg =
          Graph.constrain gg inst_term (Graph.Type ("%call", inst_params))
        in
        gg
      in
      (term, gg)
  ...
  | Let (var, rhs) as letexpr ->
      ...
      let result =
        ...
        | None -> (term, gg)
        | Some vartype ->
            (term, Graph.constrain gg term (type_to_constraint vartype))
      in
      result
  | Member mem as member ->
      let baseterm, gg = createGraph g mem.base in
      let term, gg =
        Graph.addTerm gg (baseterm.name ^ "::" ^ mem.memberName) member
      in
    ...
    (fun term cons ->
      ...
      | Func info -> (
          ...
          try
            ...
            | Ast.Parameterized ("Func", params) ->
                info.ret_type <- List.hd @@ List.rev params
            | _ -> show ()
          with _ -> show () )
      ...
      | TupleDef _ | Block _ | FloatLiteral _ | IntLiteral _ | Empty -> ()
      | StringLiteral _ ->
          ()
      ...
      | Call
          ( {
              callee =
                Var
                  ( {
                      target = Some (Multifunc ({ name = _mf_name; _ } as mf));
                      _;
                    } as cinfo );
              _;
            } as _call ) ->
          ...
          | _ ->
              if Graph.config.debug then
                Printf.printf "%s (%s) --> %s\n"
                  ...
                  (* (Ast.nodeName callee) *)
                  ( match var.target with
                  | Some n -> Ast.nodeName n
                  | _ -> "none" )
                  (Graph.cons_to_string cons) )
      | _ ->
          if Graph.config.debug then
...
let run m =
  ...
  let arith_op_type =
    Graph.Type ("Func", [ Graph.Free 0; Graph.Free 0; Graph.Free 0 ])
  in
  let bool_op_type =
    Graph.Type ("Func", [ Graph.Free 0; Graph.Free 0; Graph.Type ("Int1", []) ])
  in
  let fold_op_is optype gg o =
    let op, gg = Graph.addTerm gg o Empty in
    Graph.constrain gg op optype
  in
  let gg =
    List.fold_left (fold_op_is arith_op_type) gg [ "+"; "*"; "-"; "/"; "%" ]
  in
  let gg =
    List.fold_left (fold_op_is bool_op_type) gg
      [ "="; "<"; ">"; ">="; "<="; "!=" ]
  in
  let gg =
